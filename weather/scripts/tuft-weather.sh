...
threads=$(nproc)  # Adjust concurrency if needed
running_jobs=0

mkdir -p plots/tmp

awk -F '\t' '{print $6}' "$input" | sort -u | while IFS= read -r city; do
    (
    ...
    # Optional: throttle to $threads background jobs
    ((running_jobs+=1))
    if (( running_jobs >= threads )); then
        wait -n  # wait for one to finish before launching more
        ((running_jobs-=1))
    fi
done
